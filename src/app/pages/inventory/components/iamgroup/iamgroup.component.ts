...
@Component({
  selector: 'iamgroup',
  templateUrl: './iamgroup.html',
  styleUrls: ['./iamgroup.scss']
})
...
export class IAMGROUP {
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_iam_group").subscribe(data => {
      this.rdsdata(data);
  ...
  rdsdata(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [{title: 'CreateDate',data: 'CreateDate'},{title: 'GroupName',data: 'GroupName'},{title: 'AccountName',data: 'AccountName'
     }],
      data: this.data,

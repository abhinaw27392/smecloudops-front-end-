...
@Component({
    selector: 'bastionhost',
    styleUrls: ['./bastionhost.scss'],
    templateUrl: './bastionhost.html'
})
export class BASTIONHOST implements OnInit {
     public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_bastion_hosts").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [ { title: 'Name', data: 'Name' },{ title: 'Status', data: 'Status' },{ title: 'PublicIpAddress', data: 'PublicIpAddress' },{ title: 'PrivateIpAddress', data: 'PrivateIpAddress' },

...
@Component({
  selector: 'dbencryption',
  templateUrl: './dbencryption.html',
  styleUrls: ['./dbencryption.scss']
})

export class DBENCRYPTION implements OnInit{
   public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_db_encryption").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [{ title: 'ResourceName', data: 'ResourceName' },{ title: 'ResourceIdentifier', data: 'ResourceIdentifier' },

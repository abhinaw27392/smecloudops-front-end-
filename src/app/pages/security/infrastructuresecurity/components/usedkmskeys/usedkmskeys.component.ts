...
@Component({
  selector: 'usedkmskeys',
  templateUrl: './usedkmskeys.html',
  styleUrls: ['./usedkmskeys.scss']
})

export class USEDKMSKEYS implements OnInit {
  public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_kms_search").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [{ title: 'ResourceName', data: 'ResourceName' },
      { title: 'ResoureIdentifier', data: 'ResoureIdentifier' },

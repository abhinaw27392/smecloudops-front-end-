...
@Component({
  selector: 's3logging',
  templateUrl: './s3logging.html',
  styleUrls: ['./s3logging.scss']
})

export class S3LOGGING implements OnInit{
    public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_s3_logging").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [{ title: 'BucketName', data: 'BucketName' },

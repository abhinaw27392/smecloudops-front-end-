...
@Component({
  selector: 'elbcipherversion',
  templateUrl: './elbcipherversion.html',
  styleUrls: ['./elbcipherversion.scss']
})

export class ELBCIPHERVERSION implements OnInit{
   public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_elb_with_cipher").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [{ title: 'LoadBalancerName', data: 'LoadBalancerName' },{ title: 'SslPolicyName', data: 'SslPolicyName' },

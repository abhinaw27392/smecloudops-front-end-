...
@Component({
  selector: 'publicami',
  templateUrl: './publicami.html',
  styleUrls: ['./publicami.scss']
})

export class PUBLICAMI implements OnInit{
   public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_ami_public").subscribe(data => {
      this.ec2data(data);
...
// select ImageId,AccountName,RegionName from ami_public
    this.dtOptions = {
      pagingType: 'full_numbers',
      pageLength: 25,
      columns: [ { title: 'ImageId', data: 'ImageId' },

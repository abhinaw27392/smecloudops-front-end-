...
@Component({
  selector: 'amidetails',
  templateUrl: './amidetails.html',
  styleUrls: ['./amidetails.scss']
})

export class AMIDETAILS implements OnInit{
   public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_ami_details").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [{ title: 'InstanceName', data: 'InstanceName' }, { title: 'InstanceId', data: 'InstanceId' },
      { title: 'ImageId', data: 'ImageId' },{ title: 'ImageName	', data: 'ImageName' },

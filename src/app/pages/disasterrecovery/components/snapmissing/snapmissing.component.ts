...
@Component({
  selector: 'app-snapmissing',
  templateUrl: './snapmissing.component.html',
  styleUrls: ['./snapmissing.component.scss']
})
export class SnapmissingComponent implements OnInit {
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_vol_snap_missing").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [{ title: 'VolumeId', data: 'VolumeId' },{ title: 'AccountName', data: 'AccountName' },

...
@Component({
  selector: 'unusedebs',
  templateUrl: './unusedebs.html',
  styleUrls: ['./unusedebs.scss']
})

export class UNUSEDEBS implements OnInit {
  public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_unused_ebs").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [{ title: 'VolumeID', data: 'VolumeID' }, { title: 'Size', data: 'Size' }, { title: 'CreateTime', data: 'CreateTime' },

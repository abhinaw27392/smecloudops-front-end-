...
@Component({
  selector: 'unusedec2',
  templateUrl: './unusedec2.html',
  styleUrls: ['./unusedec2.scss']
})

export class UNUSEDEC2 implements OnInit {
  public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_unused_ec2").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [{ title: 'Name', data: 'Name' },{ title: 'InstanceId', data: 'InstanceId' },{ title: 'CPUUtilization', data: 'CPUUtilization' },
      { title: 'InstanceType', data: 'InstanceType' },{ title: 'TotalCost', data: 'TotalCost' },

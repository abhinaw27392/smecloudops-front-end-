...
@Component({
  selector: 'unusedelb',
  templateUrl: './unusedelb.html',
  styleUrls: ['./unusedelb.scss']
})

export class UNUSEDELB implements OnInit {
  public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_unused_elb").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [{ title: 'LoadBalancerName', data: 'LoadBalancerName' }, { title: 'CreatedDate', data: 'CreatedDate' },

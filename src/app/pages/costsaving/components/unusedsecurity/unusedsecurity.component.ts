...
@Component({
  selector: 'unusedsecurity',
  templateUrl: './unusedsecurity.html',
  styleUrls: ['./unusedsecurity.scss']
})

export class UNUSEDSECURITY implements OnInit {
   public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_unused_securitygroup").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [{ title: 'GroupId', data: 'GroupId' },{ title: 'GroupName', data: 'GroupName' },

...
@Component({
    selector: 'cisaudits',
    styleUrls: ['./cisaudits.scss'],
    templateUrl: './cisaudits.html'
})
export class CISAUDITS implements OnInit {
     public data = [];
  public update_time;
  ...
  ngOnInit() {
    this.http.get("http://localhost:3000/get_cis_audits").subscribe(data => {
      this.ec2data(data);
  ...
  ec2data(res) {
    ...
    this.dtOptions = {
      ...
      pageLength: 25,
      columns: [ { title: 'Hostname', data: 'Hostname' },{ title: 'Score', data: 'Score' },{ title: 'VpcName', data: 'VpcName' },
